...
export default function ProductCard({prod}: Props) {
  ...
  return (
    <div className='card card_home animate__animated animate__fadeIn animate__delay-0.5s '>
      ...
      <div style={{height:250, backgroundColor:'gray'}} className='card-body bg-dark text-white card_mota animate__animated animate__fadeInLeft animate__delay-1s'>
        <h2>{prod.tenKhoaHoc.length>30 ? prod.tenKhoaHoc.slice(0,30) + '...':prod.tenKhoaHoc}</h2>
        <h3>Lượt xem:{prod.luotXem}</h3>
        <p>{prod.moTa.length>100 ? prod.moTa.slice(0,100) + '...' :prod.moTa}</p>
      </div>
      <div className='button_productCard'>
           <button className='btn btn-success btn-primary animate__animated animate__fadeInRight animate__delay-1s '  onClick={() => {
